...
class GatedTimeSeriesAttentionLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, dropout=0.1):
        ...
        # Gating mechanism: determines how to fuse cross-attention and self-attention
...
class EnvAttn(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, num_layers, n_features, n_frames, dropout=0.1):
        super(EnvAttn, self).__init__()
        self.previous_frame_embedding = nn.Linear(10, d_model)
        self.current_frame_embedding = nn.Linear(10, d_model)
        self.prev_joints_embedding = nn.Linear(7, d_model)
        self.current_joint_embedding = nn.Linear(7, d_model)
    ...
    def forward(self, x):
        ...
        previous_frames = self.previous_frame_embedding(previous_frames)
        current_frame = self.current_frame_embedding(current_frame)

        combine_cross = self.cross_attention_layer(current_frame, previous_frames)
        self_attn_output = self.self_attention_layer(combine_cross)
        out = self.fc_out(combine_cross.squeeze(1))

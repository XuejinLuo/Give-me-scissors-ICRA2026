...
def visualise_images(frames_devices):
	for (device_info, frame) in frames_devices.items():
		device = device_info[0] #serial number
		color_image = np.asarray(frame[rs.stream.color].get_data())
		filtered_depth_frame = post_process_depth_frame(frame[rs.stream.depth], temporal_smooth_alpha=0.1, temporal_smooth_delta=80)	
		filtered_depth_frame = np.asarray( filtered_depth_frame.get_data())
		depth_mapped_image = cv2.applyColorMap(cv2.convertScaleAbs(filtered_depth_frame, alpha=0.03), cv2.COLORMAP_JET)
...
def transform_point_cloud(point_cloud, transform_matrix):
	...
	homogeneous_point_cloud = np.vstack([point_cloud, np.ones((1, N))])
	...
	# transformed_homogeneous = np.dot(homogeneous_point_cloud.transpose(), transformation_matrix)
	transformed_homogeneous = np.dot(transformation_matrix, homogeneous_point_cloud)

	transformed_point_cloud = transformed_homogeneous[:3, :]

	return transformed_point_cloud


def calculate_pointcloud(frames_devices, calibration_info_devices, output_file='output/point_cloud.txt'):
	point_cloud_cumulative = np.array([-1, -1, -1]).transpose()
	point_cloud_reshaped = []
	for (device_info, frame) in frames_devices.items() :
		device = device_info[0]
		# Filter the depth_frame using the Temporal filter and get the corresponding pointcloud for each frame
		filtered_depth_frame = post_process_depth_frame(frame[rs.stream.depth], temporal_smooth_alpha=0.1, temporal_smooth_delta=80)	
		...
		point_cloud = calibration_info_devices[device][0].apply_transformation(point_cloud)
		point_cloud_cumulative = np.column_stack((point_cloud_cumulative, point_cloud))
		#TODO: 这里我不确定需不需要补偿一个RGB和深度的矩阵，理论上是要的
		# transformed_point_cloud = transform_point_cloud(point_cloud, calibration_info_devices[device][2])
		if device == '337122071679':
			point_cloud_reshaped = point_cloud.reshape(3, 480, 640)
	#- print
	# print("Point cloud reshaped shape:", point_cloud_reshaped.shape)
	# point_240_320 = point_cloud_reshaped[:, 240, 320]
	# print("Point at (240, 320):", point_240_320)
	#- save
	# np.savetxt(output_file, point_cloud_cumulative.T, delimiter=' ', fmt='%.6f')
	point_cloud_cumulative = np.delete(point_cloud_cumulative, 0, 1)
	return point_cloud_cumulative, point_cloud_reshaped

...
class DeviceManager:
    ...
    def enable_emitter(self, enable_ir_emitter=True):
        ...
        for (device_serial, device) in self._enabled_devices.items():
            ...
            print("depth_scale: ", sensor.get_depth_scale())
    ...
    def wait_for_frames(self):
        """
        Wait for frames from the enabled Intel RealSense devices. This will block until at least one frame from each
        device is available. If temporal post-processing is enabled, the depth stream is averaged over a certain amount of frames.
        ...
        for (serial, device) in self._enabled_devices.items():
            streams = device.pipeline_profile.get_streams()
            frameset = device.pipeline.wait_for_frames()  # blocks until a frameset is available
            if frameset.size() == len(streams):
                dev_info = (serial, device.product_line)
                frames[dev_info] = frameset

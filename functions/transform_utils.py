...
def transform_points(points, T, offset):
    """
    Transform a set of points from one coordinate system to another.

    Parameters:
        points (np.ndarray): Coordinate array of points, shape (N, 3), where N is the number of points.
        T (np.ndarray): 4x4 transformation matrix.

    Returns:
        np.ndarray: Coordinate array of transformed points, shape (N, 3).
    """
    if isinstance(points, list):
        points = np.array(points)
    N = points.shape[0]
    points_homogeneous = np.hstack([points, np.ones((N, 1))])
    transformed_points_homogeneous = np.dot(T, points_homogeneous.T).T
    transformed_points = transformed_points_homogeneous[:, :3]
    transformed_points += offset # Adjust y-coordinate to match the camera coordinate system
    return transformed_points.tolist()

def transform_points_from_robot1_to_robot2(points, T_offset_robot2):
    """
    将一组点从机器人1的坐标系转换到机器人2的坐标系。

    参数:
        points (np.ndarray): 点的坐标数组，形状为 (N, 3)，其中 N 是点的数量。
        T1_to_2 (np.ndarray): 4x4 的变换矩阵, 表示从机器人1到机器人2的变换。

    返回:
        np.ndarray: 转换后的点的坐标数组，形状为 (N, 3)。
    """
    if isinstance(points, list):
        points = np.array(points)
    if points.ndim == 1:
    ...
    N = points.shape[0]
    points_homogeneous = np.hstack([points, np.ones((N, 1))])
    transformed_points_homogeneous = np.dot(T_offset_robot2, points_homogeneous.T).T
    transformed_points = transformed_points_homogeneous[:, :3]

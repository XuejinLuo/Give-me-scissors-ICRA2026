...
with open("output/origin_joint_angles.txt", "w") as f:
    f.write("")
with open("output/ICRA/fcl_input.txt", "w") as f:
    f.write("")
with open("output/ICRA/dualarm_fcl_input.txt", "w") as f:
    f.write("")
with open("output/ICRA/dualarm_ee_pose.txt", "w") as f:
    f.write("")
with open("output/ICRA/env_min_distance.txt", "w") as f:
    f.write("")
...
class ICRAsimulation:
    def __init__(self, args):
        ...
        self.Init_joint_robot1 = np.array([0, -0.7854, 0, -2.3562, 0, 1.57, 0.7854])
        ...
        Tep_robot1 = SE3.Trans(0.386 - radius * np.sin(omega * cnt * 1.5 + 3 * np.pi / 12), -0.1 - 2 * radius * np.cos(omega * cnt * 1.5 + 3 * np.pi / 12), 0.200) * SE3.OA([0, -1, 0], [0, 0, -1]) # OA: x, z
        Tep_robot2 = SE3.Trans(0.506 - radius * np.sin(omega * cnt * 1.5 + 7 * np.pi / 6), 0.15 - 2 * radius * np.cos(omega * cnt * 1.5 + 7 * np.pi / 6), 0.200) * SE3.OA([0, -1, 0], [0, 0, -1]) # OA: x, z
        ...
        # while True:
            ...
            if len(self.history_joint_state) >= 4:
                ...
                if self.args.sim:
                    ...
                    self.robot_simulation.auto_sphere1_position(cnt)
                    self.robot_simulation.auto_sphere2_position(cnt)
                    ...
                    Tep_robot1 = SE3.Trans(0.386 - radius * np.sin(omega * cnt * 1.5), -0.2 - 2 * radius * np.cos(omega * cnt * 1.5), 0.200) * SE3.OA([0, -1, 0], [0, 0, -1]) # OA: x, z
                    Tep_robot2 = SE3.Trans(0.506 + radius * np.sin(omega * cnt - np.pi/2), 0.15 + 2 * radius * np.cos(omega * cnt - np.pi/2), 0.200) * SE3.OA([0, -1, 0], [0, 0, -1]) # OA: x, z
                    sol_robot1 = self.robot.ik_LM(Tep_robot1, q0=self.last_joints_command_robot1)
                    sol_robot2 = self.robot.ik_LM(Tep_robot2, q0=self.last_joints_command_robot2)
                    self.robot1_pos_target = Tep_robot1.t
                    self.robot2_pos_target = Tep_robot2.t
                ...
                with open('output/ICRA/dualarm_fcl_input.txt', 'a') as dualarm_fcl_input:
                    dualarm_fcl_input.write(
                        ', '.join(f"{value:.3f}" for value in optimized_last_step[:7]) + ", " +
                        ', '.join(f"{value:.3f}" for value in optimized_last_step[7:]) + "\n"
                    )
                with open('output/ICRA/fcl_input.txt', 'a') as fcl_input:
                    fcl_input.write(' '.join(map(str, optimized_last_step[:7])) + " " + " ".join(map(str, self.env_nearest_point_robot1)) + "\n")
                with open('output/ICRA/dualarm_ee_pose.txt', 'a') as dualarm_ee_pose:
                    pose1_str = ', '.join(f"{x:.5f}" for x in self.robot1_pos_now )
                    pose2_str = ', '.join(f"{x:.5f}" for x in self.robot2_pos_now )
        ...
        while self.robot_interface._is_running:
            ...
            if rclpy.ok():
                ...
                if self.curr_joint_pos is not None:
                    ...
                    if self.args.sim:
                        self.env_nearest_point_robot1 = self.env_nearest_sphere1
                        self.env_nearest_point_robot2 = np.linalg.inv(self.T_offset_robot2) @ np.append(self.env_nearest_sphere2 * 0.001, 1)
                        ...
                        with open('output/ICRA/env_min_distance.txt', 'a') as env_min_distance_file:
                            env_min_distance_file.write(f"{self.env_nearest_point_robot1} {self.robot_interface.min_distance_robot1} {self.env_nearest_point_robot2} {self.robot_interface.min_distance_robot2}\n")
    ...
    def _sim_get_robot_data(self):
        ...
        grab_state = np.where((robot_1_grab_state == 0) | (robot_2_grab_state == 0), 0, 1)
        self.env_nearest_sphere1 = np.squeeze(self.robot_simulation.sphere1_position * 1000) 
        self.env_nearest_sphere2 = np.squeeze(self.robot_simulation.sphere2_position * 1000) 
        return curr_joint_pos, grab_state
